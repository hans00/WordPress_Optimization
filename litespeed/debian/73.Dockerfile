...
RUN apt-get update \
	...
	&& apt-get install -y openlitespeed lsphp73 lsphp73-* \
# freeze
...
# setup lsphp
	...
	&& ln -sf /usr/local/lsws/lsphp73/bin/lsphp /usr/bin/lsphp \
# install phpredis
	&& (\
		cd /tmp/; \
		...
		make; \
		make install; \
		echo "extension=redis.so" > /usr/local/lsws/lsphp73/etc/php/7.3/mods-available/redis.ini; \
		)\
# setup the config
